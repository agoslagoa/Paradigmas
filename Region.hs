...
addCityToRegion :: Region -> City -> Region
...
createLinkBetweenCities :: Region -> City -> City -> Quality -> Region
...
createTunnelBetweenCities region cities@(cityA:cityB:rest) =
    case findLink region cityA cityB of
        Just link -> updatedRegion
            where
                links = map (\(c1, c2) -> fromJust $ findLink region c1 c2) $ zip cities (tail cities)
